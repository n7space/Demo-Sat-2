...
void
device_interface_deliver_function(const uint8_t* const data, const size_t data_size)
{
    (void)data_size;
...
main()
{
    ...
    Serial_CCSDS_Linux_Conf_T device{
        "/tmp/serialTest", Serial_CCSDS_Linux_Baudrate_T_b9600,
        Serial_CCSDS_Linux_Parity_T_odd, 8, 0, {}
    };

    serial.driver_init(BUS_INVALID_ID, DEVICE_INVALID_ID, &device, nullptr);

    sendThread.start(&PacketSender::SendThreadMethod1, &serial);
